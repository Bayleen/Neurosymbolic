...
def get_timestamps_from_csv(csv_path):
    ...
    timestamp = df.iloc[:, 0].apply(lambda x: str(int(float(x)))).tolist()
    return timestamp
...
if __name__ == "__main__":
    ...
    # ----Configuration ----
    label_cvs_path = r"E:\UTK\Reseach\Publication\Workshop\ICRA2025_workshop_dataset_HumanRobotCorr-main\dataset_depth\dataset_depth\robot\cam_104122061850\pick\0004\Labelled_Robot_004.csv"

    image_root_folder = r"E:\UTK\Reseach\Publication\Workshop\ICRA2025_workshop_dataset_HumanRobotCorr-main\dataset_depth\dataset_depth\robot"
    ...
    for timestamp in timestamps:
        ...
        if not depth_v2_image_path or not rgb_image_path:
            print(f"Missing one or both images for timestamp: {timestamp}!")
            continue

        user_folder_path = Path(depth_v2_image_path).parent.parent
        voxel_grid_path = user_folder_path / 'voxel_grids' / f"{timestamp}.npy"
    
        if voxel_grid_path.exists():
          print(f"Voxel already exists: {voxel_grid_path}, skipping.")
          continue
        ...
        #print("Looking for:", image_filename)
        #print("Found RGB:", rgb_image_path)
        #print("Found Depth v2:", depth_v2_image_path)
        ...
        if depth_v2_image_path and rgb_image_path:
            ...
            #save file
            output_dir = user_folder_path / 'voxel_grids'
            output_dir.mkdir(parents=True, exist_ok=True)
